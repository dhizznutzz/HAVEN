...
export function ChallengeCard({ challenge, onJoin }: ChallengeCardProps) {
  ...
  const handleEnroll = () => {
    setJoined(true);
    setModalOpen(false);
    toast.success(`Enrolled in "${challenge.title}"! +${challenge.xp} XP earned 🏆`);
    onJoin?.(challenge.xp);
  };
    ...
    <>
      <div className="rounded-xl border border-gray-100 bg-white p-4 hover:border-gray-200 transition-colors">
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <button
            onClick={() => { if (!joined) setModalOpen(true); }}
            disabled={joined}
          ...
          >
            {joined ? 'Enrolled ✓' : 'Join Challenge'}
          </button>
        </div>
      </div>
